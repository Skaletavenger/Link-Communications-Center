...
export default function MTNPayModal({ open, onClose, onSuccess, product }: Props) {
  const [phone, setPhone] = useState('')
  const [amount, setAmount] = useState(product?.price || 0)
  const [reference, setReference] = useState('')
  const [loading, setLoading] = useState(false)
  const [status, setStatus] = useState<string | null>(null)
  const [mtnTxId, setMtnTxId] = useState<string | null>(null)
  ...
  const startPayment = async () => {
    ...
    try {
      ...
      // MTN response includes transactionId
      const mtnId = data?.transactionId || data?.data?.transactionId || null
      setMtnTxId(mtnId)
      setStatus('PENDING')
      ...
      pollRef.current = window.setInterval(async () => {
        ...
        if (!mtnId) {
          ...
          setLoading(false)
          setError('Missing MTN transaction id')
          return
        }

        try {
          const st = await fetch(`/api/mtn/status/${encodeURIComponent(mtnId)}`)
          const sd = await st.json()
          const current = sd?.data || sd
          const state = sd?.status || current?.status || null
          if (state) setStatus(state)
          // consider successful statuses
          const okStates = ['SUCCESSFUL', 'SUCCESS', 'COMPLETED', 'APPROVED']
          if (okStates.includes((state || '').toString().toUpperCase())) {
            clearInterval(pollRef.current!)
            setLoading(false)
            onSuccess?.(mtnId || reference)
            return
          }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      >
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-bold" style={{ color: 'white' }}>Pay with MTN MoMo</h3>
        ...
        <div className="space-y-3">
          ...
          {mtnTxId && (
            <div className="text-sm text-white/80">MTN transaction ID: {mtnTxId}</div>
          )}
