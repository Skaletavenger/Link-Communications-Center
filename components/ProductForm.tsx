...
export default function ProductForm({ onSave }: { onSave: (p: Product) => void }) {
  ...
  return (
    <form className="grid gap-2 bg-white/3 p-4 rounded" onSubmit={(e) => { e.preventDefault(); onSave(form); setForm({ name: '', model: '', category: 'Surveillance Cameras', price: 0, stockQuantity: 0, image: '/placeholder.svg' }); }}>
